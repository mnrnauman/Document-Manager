...
export default function QuotationEditor() {
  ...
  const addFromCatalog = (catalogIndex: string) => {
    ...
    setCatalogValue("")
    toast({ title: "Item added", description: `${product.name} added to quotation` })
  }
  ...
  const removeTerm = (index: number) => {
    if (formData.terms.length > 1) {
      ...
      toast({ title: "Cannot remove term", description: "You must have at least one term", variant: "destructive" })
    }
  }
  ...
  const handleDownloadPDF = async () => {
    ...
    if (element) {
      try {
        await generatePDF(element, `Quotation_${formData.quotationNumber}.pdf`)
    ...
    if (printRef.current) {
      ...
      if (printWindow) {
        const styleSheets = Array.from(document.querySelectorAll("style, link[rel='stylesheet']"))
          .map((el) => el.outerHTML)
          .join("\n")
        printWindow.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Quotation</title>${styleSheets}<style>@media print{@page{size:A4;margin:10mm;}body{margin:0;background:white;}}</style></head><body style="background:white;">${printContent}</body></html>`)
  ...
  const handleSave = () => {
    ...
    toast({ title: "Draft saved", description: "Your quotation draft has been saved locally" })
  }
  ...
  const handleLoad = () => {
    ...
    if (savedData) {
      ...
      toast({ title: "No saved draft", description: "No saved quotation draft was found", variant: "destructive" })
    }
  }
  ...
  const PreviewContent = ({ forPDF = false }: { forPDF?: boolean }) => {
    ...
    <div className={forPDF ? "print-optimized bg-white" : "w-full p-6"} style={forPDF ? { padding: "20px 28px", fontSize: "11px" } : {}}>
      ...
      <div className={`text-center ${forPDF ? "my-2" : "my-3"}`}>
        <h2 className={`font-bold text-[#1e40af] uppercase ${forPDF ? "text-base" : "text-xl"}`}>Quotation</h2>
      </div>
      ...
      <div className={`flex justify-between gap-3 ${forPDF ? "mb-2" : "mb-4"}`}>
        <div className={`bg-gray-50 rounded-md border border-gray-100 flex-1 ${forPDF ? "p-2" : "p-3"}`}>
          <h3 className={`font-semibold text-[#1e40af] ${forPDF ? "text-xs mb-0.5" : "text-sm mb-1"}`}>Client Information:</h3>
        ...
        <div className={`bg-gray-50 rounded-md border border-gray-100 w-1/3 ${forPDF ? "p-2" : "p-3"}`}>
          <h3 className={`font-semibold text-[#1e40af] ${forPDF ? "text-xs mb-0.5" : "text-sm mb-1"}`}>Quotation Details:</h3>
          <div className="grid grid-cols-2 gap-x-1" style={{ fontSize: "10px" }}>
            ...
            <p>{formData.validUntil ? new Date(formData.validUntil).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }) : ""}</p>
          </div>
        </div>
      </div>
      ...
      <div className={forPDF ? "mb-2" : "mb-4"}>
        <table className="w-full border-collapse">
          <thead>
            <tr>
              <th className={`${p} text-left bg-[#f97316] text-white font-semibold w-1/2 ${rowText}`}>Item Description</th>
              <th className={`${p} text-center bg-[#f97316] text-white font-semibold w-1/6 ${rowText}`}>Qty</th>
              <th className={`${p} text-right bg-[#f97316] text-white font-semibold w-1/6 ${rowText}`}>Unit Price</th>
              <th className={`${p} text-right bg-[#f97316] text-white font-semibold w-1/6 ${rowText}`}>Total</th>
            </tr>
          </thead>
          <tbody>
      ...
      {/* Terms */}
      <div className={forPDF ? "mb-2" : "mb-4"}>
        <h3 className={`font-semibold text-[#1e40af] ${forPDF ? "text-xs mb-0.5" : "text-sm mb-1"}`}>Terms and Conditions</h3>
        <ul className="list-disc pl-4 text-gray-600" style={{ fontSize: "10px" }}>
      ...
      <div className={`grid grid-cols-2 gap-6 ${forPDF ? "mt-3 mb-2" : "mt-6 mb-4"}`}>
        ...
        <div>
          <p className="text-gray-700 mb-1" style={{ fontSize: "10px" }}>Accepted by (Client):</p>
          <div className="border-b border-gray-300 h-5 mb-1" />
          <p className="text-gray-500" style={{ fontSize: "9px" }}>Name, Signature & Date</p>
        </div>
      </div>
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input type="text" id="quotationNumber" name="quotationNumber" value={formData.quotationNumber} onChange={handleChange} />
            </div>
            <div>
            ...
            <div>
              ...
              <Input type="date" id="validUntil" name="validUntil" value={formData.validUntil} onChange={handleChange} />
            </div>

            {/* Items — at the top, with scrollable list */}
              ...
              {formData.terms.map((term, index) => (
                <div key={index} className="flex items-start gap-2 mb-2">
                  <Textarea
                    ...
                    className="resize-none text-sm"
                  />
                  <Button type="button" variant="ghost" size="sm" onClick={() => removeTerm(index)} className="h-8 w-8 p-0 mt-1">
                    <Trash2 size={14} className="text-red-500" />
                  </Button>
                </div>
              ))}
              <Button type="button" variant="outline" onClick={addTerm} className="w-full mt-2 flex items-center justify-center gap-2">
                <Plus size={16} />
                Add Term
              </Button>
            </div>
            ...
            <div>
              ...
              <Input type="text" id="authorizedBy" name="authorizedBy" placeholder="Your Name" value={formData.authorizedBy} onChange={handleChange} />
            </div>
